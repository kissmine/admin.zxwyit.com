<template>
  <!-- 4、试卷总预览与修改 -->
  <div class="view_buttom">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>
          一、选择题（本题共{{ choiceData.length }}道小题，共{{ sumChoice }}/{{ sumData }}分）
        </span>
      </div>
      <ol class="add-ol">
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>
          二、填空题（本题共{{ GapData.length }}道小题，共{{ sumGap }}/{{ sumData }}分）
        </span>
      </div>
      <ol class="add-ol">
        <li v-for="(item,index) in GapData" :key="index" class="li-number">
          ...
          <el-form status-icon @submit.native.prevent v-show="!item.redactShow">
            <el-form-item
              label="题干"
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>
          三、问答题（本题共{{ essayData.length }}道小题，共{{ sumEssay }}/{{ sumData }}分）
        </span>
      </div>
      <ol class="add-ol">
...
export default {
  ...
  data() {
    return {
      ...
      essayData: [], //填空题的信息
      cancelData3: [], //取消问答题修改
      Allofthescores: { //三大题目的分值，用来传剃给完成视图的
        cumchoice: "0", //选择题总分
        cumgap: "0", //填空题总分
        cumessay: "0", //问答题总分
        cumdataUp: "0" //所有题目的总分
  ...
  methods: {
    ...
    formMessage(code, msg) {
      let _this = this;
      let type = "warning";
      let message = "其它错误";// 返回代码 0：没有改变 1：成功 -1：系统异常 -2：参数错误 除此之外就是其它错误
      switch (code) {
        case -1:
          message = msg;
          break;
  ...
  computed: {
    ...
    */
    sumGap() {
      ...
      _this.GapData.forEach(item => {
        ...
        item.tpqScore = sum;
        this.Allofthescores.cumgap = sum;
      });
      return sum;
    },
    /**
    ...
    */
    sumData() {
      ...
      _this.Allofthescores.cumdataUp = sum;
      this.$emit("AllsumUp", this.Allofthescores);//将获取到的分数发射出去
      return sum;
    }
  }
};
...
.li-number {
  ...
  margin-bottom: 20px;
  // box-shadow: #ebeef5 0px 0px 10px 2px;
  background: #fff;
}
.ul_choice {
  padding-left: 0px;
